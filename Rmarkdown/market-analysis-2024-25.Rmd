    ...
    sidebarLayout(
      sidebarPanel(
        ...
        pickerInput(
          ...
          selected = economic_factor_choice[3],
          options = list(style = "btn-danger"),
          width = "100%"
        ),
...
economics_graph_server <- function(input, output){
  ...
  economics_filtered_graph <- 
    reactive({
      ...
      } else if(input$economics_selection == economic_factor_choice[4]){
        temp %>% 
      ...
      } else if(input$economics_selection == economic_factor_choice[5]){
        temp %>% 
...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-#
#FILTER VECTORS
supply_graph_choice <- c("Volumes w.r.t Sale Revenue", "Company Shares", "N° Enterprises", "N° Employees",
                         "Value Added in Production", "Gross Operating Surplus", "Average Wages and Salaries")
supply_graph_code <- list(clean_volume_price, clean_company_shares, clean_enterprise_n, clean_employee_n,
                          clean_val_added, clean_op_surplus, clean_wages)
...
supply_graph_server <- function(input, output){
    ...
    if(input$supply_selection == supply_graph_choice[1]){
        ...
        tagList(
            ...
            inputId = "company_share_selection",
              ...
              choices = c("Average Share Value over Time", "Market Concentration"),
              selected = "Average Share Value over Time",
              options = list(style = "btn-danger"),
              width = "100%"
            ),
            chooseSliderSkin("Flat", color = '#db240c'),
  ...
  supply_filtered_graph <- reactive({
    ...
    if(input$supply_selection == supply_graph_choice[1]){
      ...
      if(input$company_share_selection == "Average Share Value over Time"){
  ...
  output$supply_highchart <- renderHighchart({
    if(input$supply_selection == supply_graph_choice[1]){
      supply_filtered_graph() %>% 
        ...
        hc_title(text = "Snack Production Volumes w.r.t Average Price") %>%
        lnplot_custom(.) %>%
        hc_yAxis(title=list(
        ...
        hc_tooltip(.,
             headerFormat = "",
             pointFormat =
               "<b>{point.name}</b><br>Retail Volume: {point.x:.f}k tonnes<br>Average Price: {point.y:.1f} €<br>Retail Value RSP: {point.Retail Value RSP:.1f} mil €"
             ) %>% 
        hc_caption(
          text = "<b>Retail Sale Revenue/Volumes</b> = relationship expressing a form of average price.",
          style = list(marginTop = "0px")
        )
    } 
    else if(input$supply_selection == supply_graph_choice[2]){
        if(input$company_share_selection == "Average Share Value over Time"){
          supply_filtered_graph() %>% 
            ...
            hc_title(text = "Average Share Value over Time") %>%
            lnplot_custom(.) %>% 
            hc_yAxis(title=list(
            ...
            hc_caption(
              text = "<b>N-Indexed Average Share Value</b> = average value of the first n companies' share, relative to the snack market for any year. Indicative of competitiveness for small intervals.",
              style = list(marginTop = "0px")
            )
        } else if (input$company_share_selection == "Market Concentration"){
            ...
            highchart() %>%
              ...
              hc_yAxis(title = list(text = "<b>Aggregate Share Value</b>", align = "middle", min = 0, max = 50,
                                    labels = list(
                                              formatter = JS("function() {
                                                  if (this.value >= 0) {
                                                    return (this.value) + '€';
              ...
              hc_caption(
                text = "<b>N-Indexed Market Concentration</b> = sum of share value of the first n companies in share value, indicator of concentration of market activities.",
                style = list(marginTop = "0px"))
        }
    }
    else if(input$supply_selection == supply_graph_choice[3]){
      supply_filtered_graph() %>% 
        ...
        hc_caption(
          text = "<b>N° Enterprises</b> = number of enterprises active in the food manufacture market at a given time.",
          style = list(marginTop = "0px")
        )
    }
    else if(input$supply_selection == supply_graph_choice[4]){
      supply_filtered_graph() %>% 
        ...
        hc_caption(
          text = "<b>N° Employees</b> = number of employees working for enterprises active in the food manufacture market at a given time.",
          style = list(marginTop = "0px")
        )
    }
    else if(input$supply_selection == supply_graph_choice[5]){
      supply_filtered_graph() %>% 
        ...
        hc_caption(
          text = "<b>Value Added through Production</b> = increase in value brought by a production process, quantifiable as the difference between its sale price and the cost of its relative production factors.",
          style = list(marginTop = "0px")
        )
    }
    else if(input$supply_selection == supply_graph_choice[6]){
      supply_filtered_graph() %>% 
        ...
        hc_title(text = "Gross Operating Surplus") %>%
        lnplot_custom(.) %>% 
        hc_yAxis(title=list(
        ...
        hc_caption(
          text = "<b>Gross Operating Surplus</b> = enterprise profit on goods and services produced following the payment of labour costs.",
          style = list(marginTop = "0px")
        )
    }
    else if(input$supply_selection == supply_graph_choice[7]){
      supply_filtered_graph() %>% 
        ...
        hc_caption(
          text = "<b>Aggregate Labour Cost</b> = cost of labour, aggregate corporate expenditure in wages and salaries.",
          style = list(marginTop = "0px")
        )
    }
    else if(input$supply_selection == supply_graph_choice[8]){
      supply_filtered_graph() %>% 
        hchart("line", hcaes(Year, `Investment Rate`)) %>% 
        hc_title(text = "Investment Rate [%]") %>%
        lnplot_custom(.) %>% 
        hc_yAxis(title=list(
                         text="<b>Investment Rate [%]</b>", 
                         align="middle"
                       ),
                 labels = list(
                            formatter = JS("function() {
        ...
        hc_caption(
          text = "<b>Investment Rate</b> = ratio between gross tangible investment to net value added, a measure of conversion of investments into tangible profit.",
          style = list(marginTop = "0px")
        )
    }
  })
...
## Supply Side - Text {.tabset .tabset-fade}

### Supply 1/2

#### Retail Value vs Volume
...
#### Investment Rate and Business Growth
