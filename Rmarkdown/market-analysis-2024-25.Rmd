...
%>%        # Group data by Year
  summarize(Mean_Value = mean(Value, na.rm = TRUE)) # Calculate mean value


#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-#
#FILTER VECTORS
supply_graph_choice <- c("Volumes w.r.t Sale Revenue", "Company Shares")
supply_graph_code <- list(clean_volume_price, clean_company_shares)
...
supply_graph_server <- function(input, output){
  ...
  output$supply_plus_filter <- renderUI({
    if(input$supply_selection == supply_graph_choice[1]){
        tagList(
          ...
          chooseSliderSkin("Flat", color = '#db240c'),
          sliderInput(inputId = "supply_year_slider",
                      label = "Year Interval",
                      min = 2010,
                      max = 2024,
                      value = c(2010, 2024),
  ...
  supply_filtered_graph <- reactive({
    ...
    if(input$supply_selection == supply_graph_choice[1]){
      req(input$volumes_country_selection, input$supply_year_slider)
      temp <-
        temp[[1]] %>% 
        filter(Geography == as.character(input$volumes_country_selection)) %>% 
        filter(Year >= input$supply_year_slider[1], Year <= input$supply_year_slider[2])
    }
  })
...
supply_graph_server <- function(input, output){
  ...
  output$supply_highchart <- renderHighchart({
    if(input$supply_selection == supply_graph_choice[1]){
      supply_filtered_graph() %>% 
        hchart("line", hcaes(`Retail Volume (Tonnes)`, `Retail Value RSP`, name= Year)) %>% 
        hc_title(text = "Snack Production Volumes w.r.t Average Price") %>%
        lnplot_custom(.) %>%
        hc_yAxis(title=list(
                         text="<b>Value Generated in Snack Market [Retail Value RSP]</b>", 
                         align="middle"
                       ),
                 labels = list(
                            formatter = JS("function() {
        ...
        hc_xAxis(title=list(text="<b>Snack Retail Volume</b>", align="middle")) %>%
        hc_tooltip(.,
             headerFormat = "",
             pointFormat =
               "<b>{point.name}</b><br>Retail Volume: {point.x:.f} tonnes<br>Retail Value RSP: {point.y:.f} mil €<br>Ratio: {point.Price/Volume:.1f} mil €/tonnes") %>%
        hc_caption(
          text = "<b>Nominal GDP</b> = value expressing the amount of goods and services that are produced and sold through markets, at a given time, calculated using current prices.",
          style = list(marginTop = "0px")
        )
