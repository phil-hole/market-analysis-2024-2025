...
selected_nations <- c("Portugal", "Spain", "Italy")

clean_population <- 
  dsets[["portugal_population"]] %>% 
  select(-Code) %>% 
  filter(Year >= 1800 & Entity %in% selected_nations) %>% 
...
selected_population <-  
  ...
  hc_yAxis(title= list("Population"), align="middle") %>% 
  lnplot_custom(.)

selected_population_density <-
  clean_population_density %>% 
  filter(Entity == "Portugal") %>% 
  hchart("line", hcaes(`Year`, `Population Density`)) %>% 
  hc_title(text = "Portugese Population Density") %>% 
  hc_xAxis(title= list("Year"), align="middle") %>%
  hc_yAxis(title= list("Population Density"), align="middle") %>% 
  lnplot_custom(.)

selected_population_graphs <- c(selected_population, selected_population_density)
population_graph_choices <- c("Population", "Population Density")
...
population_ui <- 
  fluidPage({
    sidebarLayout(
      sidebarPanel(
        width = 3,
        selectInput("population_graph", label= "Select Graph:", 
                    choices= population_graph_choices, 
                    selected= "Population")
      ),
      mainPanel(
        width = 9,
        highchartOutput("population_placeholder")
      ),
      position = c("left", "right")
    )
  })

population_server_logic <- function(input, output){
  highchart_population_selection <- reactive({
    final_population_graph <- 
      population_graph_choices[population_graph_choices %in% input$population_graph]
  })
  
  output$population_placeholder <- renderHighchart({
    final_population_graph <- highchart_population_selection()
    final_population_graph <- final_population_graph[[1]]
    final_population_graph
  })
}

shinyApp(ui = population_ui, server = population_server_logic)
```
