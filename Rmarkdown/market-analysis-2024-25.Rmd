...
#+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-#
#FILTER VECTORS
demand_graph_code <- list(clean_demand_market_size, clean_subcategories, clean_subcategories,
                          clean_volume_parallel, clean_gdp_capita_volumes)
demand_graph_choice<- c("Percentage of Snack Revenue", "Subcategories of Case Study - Values",
...
## Vieira - Text & Stats {data-width="1000"}

### **Information** {data-height="900"}
...
```{r VIEIRA GAUGES}
highchart() %>%
  # Set chart type to solid gauge
  hc_chart(type = "solidgauge") %>%
  
  # Define the pane (background arc)
  hc_pane(
    startAngle = -90,
    endAngle = 90,
    background = list(
  ...
  hc_add_series(
    data = list(65),  # Value for the gauge
    name = "Performance",
    dataLabels = list(
...
# hc_data <- 
#   lapply(
#     names(clean_subcategories)[-1], 
#     function(company) {
#       list(name = company, data = as.numeric(clean_subcategories[[company]]))
#     }
#   ) 
# 
# highchart() %>%
...
#               hc_add_series_list(hc_data) %>%
#               barplot_custom(.) %>% 
#               hc_xAxis(categories = clean_subcategories$Category) %>%
#               hc_yAxis(title = list(text = "<b>Aggregate Share Value [%]</b>", align = "middle"),
#                        min = 0,
#                        labels = list(
#                                  formatter = JS("function() {
...
#                                 )
#                        ) %>%
              # hc_colors(c("#db240c","#FFE900", "#FFB900","#046A38", "#05fc83")) %>%
#               hc_caption(
#                 text = "<b>N-Indexed Market Concentration</b> = sum of share value of the first n companies, share value expressed in terms of % of total market value.<br>Indicative of market concentration, the greater the value for small n, the more tendentially monopolistic the market.",
#                 style = list(marginTop = "0px")
#               ) %>% 
