...
runtime: shiny
resource_files:
- Datasets/portugal_company_shares.xlsx
- Datasets/portugal_consumer_price_index.csv
- Datasets/portugal_daily_caloric_supply.csv
- Datasets/portugal_dsets_estat.xlsx
- Datasets/portugal_gdp_constant.csv
- Datasets/portugal_gdp_per_capita.csv
- Datasets/portugal_hdi.csv
- Datasets/portugal_inflation.csv
- Datasets/portugal_life_expectancy.csv
- Datasets/portugal_migrant_stock.csv
- Datasets/portugal_overweight_adults.csv
- Datasets/portugal_population.csv
- Datasets/portugal_population_density.csv
- Datasets/portugal_population_growth_rate.csv
- Datasets/portugal_population_poverty.csv
- Datasets/portugal_real_gdp.csv
- Datasets/portugal_retail_volume_price.xlsx
- Datasets/portugal_snack_subcategories.xlsx
- Datasets/portugal_unemployment_rate.csv
---
...
<!-- GITHUB REPOSITORY LINK 
     https://github.com/phil-hole/market-analysis-2024-2025.git
-->
...
#DATA IMPORTING
#when downloading the file, the working directory should automatically set to the path where the Markdown file is saved. if that does not occur, copy the Markdown path from the dropdown menu between the "" and run the single line of code.
...
clean_life_expectancy <-
  ...
  lnplot_custom(.) %>% 
  hc_yAxis(
    title = list(text = "<b>Life Expectancy  [yrs]</b>", align = "middle"), 
    labels = list(
                  formatter = JS("function() {
                      if (this.value > -1000) {
    ...
    reactive({
      ...
      if(input$economics_selection == economic_factor_choice[1]){
        temp %>% 
            ...
            hc_xAxis(title=list(text="<b>Year [Quarters]</b>", align="middle")) %>%
            hc_tooltip(.,
                 headerFormat = "",
                 pointFormat =
...
- Stable Trend (pre-2008): Unemployment rate consistently between 3-4% and 10%.
...
- Recent Trends:
  - Building rebound in labour participation, signaling recovery and growing confidence in the job market.
...
- Vieira de Castro plays a role in the CLabel+ (“clean label”) project, focusing on wellness with products like no-sugar, lactose-free, and fibre-rich options.
